...
library(plyr)
inv_comm<-make[,-c(1,2)]
inv_plot_attr <- ddply(down, .( Plot, Centroid_X,Centroid_Y),summarize, Area_ha=mean(Area_ha))
inv_plot_attr<-inv_plot_attr[,c(1:3)]

# spatial coordinates must be named x y to guarantee that they will be recognized as coordinates by the function.
colnames(inv_plot_attr)<-c("Plot", "x", "y")
...
```{r}
mobr_object <- make_mob_in(inv_comm, inv_plot_attr, latlong=TRUE)
